...
const ChatBot = () => {
  ...
  const sendMessage = async (e) => {
    ...
    try {
      // ✅ UPDATED: Sending the token so the backend knows you're a Dev
      const response = await axios.post(`${API_URL}/api/chat`, 
        { message: input },
        { 
          headers: { 
            'Content-Type': 'application/json',
            'x-auth-token': localStorage.getItem('token') // Send JWT
          } 
        }
      );
      ...
      setMessages(prev => [...prev, { 
        text: "Connection trouble. Please check if the backend is awake.", 
        isBot: true,
        isError: true 
      }]);
    } finally {
    ...
    <div className="fixed bottom-6 right-6 w-96 z-50 shadow-2xl rounded-[2rem] overflow-hidden border border-gray-100 bg-white">
      ...
      <form onSubmit={sendMessage} className="p-4 bg-white border-t border-gray-100 flex gap-2">
        <input 
          ...
          className="flex-1 bg-gray-100 p-3 rounded-xl text-sm outline-none"
        />
