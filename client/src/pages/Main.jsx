...
const Main = () => {
  ...
  // ✅ SOFTWARE LOGIC: Check if user is logged in
  ...
  useEffect(() => {
    let result = services;
    if (category !== 'All') result = result.filter(s => s.category.toLowerCase() === category.toLowerCase());
    if (searchTerm) result = result.filter(s => s.name.toLowerCase().includes(searchTerm.toLowerCase()) || s.category.toLowerCase().includes(searchTerm.toLowerCase()));
    setFilteredServices(result);
  }, [category, searchTerm, services]);

  const handleLogout = () => {
    localStorage.clear();
    navigate('/login'); // ✅ Redirects to the Login software we built earlier
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 font-sans text-gray-800">
      ...
      {/* ✅ Navbar: Now adapts based on Login Status */}
      ...
      <div className="container mx-auto px-4 max-w-5xl">
        <div className="py-12 text-center space-y-4">
            ...
            <div className="pt-6 flex flex-col items-center gap-4">
               {/* ✅ Add Service Button checks Login */}
               <button onClick={() => navigate(isLoggedIn ? "/add-service" : "/login")} className="flex items-center gap-2 bg-black text-white px-6 py-3 rounded-full font-bold shadow-lg hover:scale-105 transition transform">
